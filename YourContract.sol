...
contract YourContract is Ownable, ReentrancyGuard {
    ...
    uint public _gameNumber;
    uint public _initFee;
    uint public _feesPending;

    constructor() ReentrancyGuard() {
        _gameNumber = 1;
        _initFee = 1000000000000000;
        _feesPending = 0;
    }
    ...
    modifier isActiveGame() {
        require(idToGame[playerInfo[msg.sender].gameId].isActive == true, "Make sure you are attempting to interact with is active.");        
        _;
    }

    modifier checkCalls() { // To verify that msg.sender is attemping to join a game that has not ended
        require(playerInfo[msg.sender].isInGame == true, "You are currently not in a game.");
    ...
    // ------------------------------------------ Buyin Functions ----------------------------------------------
     ...
     function buyin(string memory name, uint inputId, uint buyinAmount) public payable isNotInGame {
        ...
        require(idToGame[inputId].endedBuyin == false, "This game's buyin period has already ended.");     
    ...
    */
    ...
    function payout() nonReentrant() external checkCalls verifyChecks {
        ...
        require(idToGame[playerInfo[msg.sender].gameId].isActive == true, "The game you are attempting to payout from is not active!");


        uint id = playerInfo[msg.sender].gameId;
        uint toBeSent = playerInfo[msg.sender].withdrawalAmount;

        idToGame[playerInfo[msg.sender].gameId].tableWithdrawalReqs--; 
        idToGame[playerInfo[msg.sender].gameId].playerCount--; 


        idToGame[playerInfo[msg.sender].gameId].gamePot -= toBeSent; 
        idToGame[playerInfo[msg.sender].gameId].etherWithdrawalReqs -= toBeSent; 
        ...
        if (idToGame[id].playerCount == 0) {
            idToGame[id].isActive = false;
        }
    ...
    function withdrawFees() public onlyOwner {
        require(_feesPending > 0, "No fees to collect.");
        _feesPending = 0;
        payable(msg.sender).transfer(_feesPending);
    }
